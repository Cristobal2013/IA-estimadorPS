...
# === Helper: guardar estimación confirmada en data/estimaciones_nuevas.csv ===
def _save_estimation_row(tipo, texto, metodo_usado, estimacion_ia, estimacion_top1, estimacion_final, horas_reales):
    ...
    row = {
        ...
        "estimacion_ia": _to_float(estimacion_ia),
        "estimacion_top1": _to_float(estimacion_top1),
        "estimacion_final": _to_float(estimacion_final),
        "horas_reales": _to_float(horas_reales),
    }
    df_out = _pd.DataFrame([row])
...
def api_accept():
    try:
        ...
        tipo = (data.get("tipo") or "").strip().lower() or "desarrollo"
        texto = (data.get("texto") or "").strip()
        metodo_usado = (data.get("metodo") or data.get("metodo_usado") or "softmax").strip()
        estimacion_ia = data.get("estimacion_ia") or data.get("horas") or None
        estimacion_top1 = data.get("estimacion_top1") or None
        estimacion_final = data.get("estimacion_final") or estimacion_ia
        horas_reales = data.get("horas_reales") or None
        _save_estimation_row(tipo, texto, metodo_usado, estimacion_ia, estimacion_top1, estimacion_final, horas_reales)
...
def api_estimate():
    ...
    texto = (data.get("texto") or "").strip()
    metodo = (data.get("metodo") or "softmax").strip().lower()
    origen = (data.get("origen") or "todos").strip().lower()

    tag = "desarrollo" if tipo == "desarrollo" else "implementacion"

    # Cargar/entrenar índice
    est = EmbeddingsFaissEstimator(tag)
    loaded = False
    try:
        loaded = est.load()
    ...
    if not loaded:
        try:
            ...
            return jsonify({"ok": False, "error": f"No se pudo cargar/entrenar índice [{tag}]: {e2}"}), 500

    # Predicción y vecinos
    est_soft_unused, neighbors_all = est.predict(texto, k=30)
    labeled = load_labeled_dataframe(tag).reset_index(drop=True)

    neighbors = _filter_neighbors_by_source(neighbors_all, labeled, origen)
    neighbors_used = _prepare_neighbors_for_estimation(neighbors, top_k=KNN_TOPK, min_sim=KNN_MINSIM)

    # Estimación
    if metodo == "top1" and neighbors_used:
        estimate = sorted(neighbors_used, key=lambda t: t[1], reverse=True)[0][2]
        metodo_usado = "top1"
    else:
        if USE_FAISS_KNN:
            estimate, _ = _estimate_with_faiss_knn(neighbors_used, min_sim=KNN_MINSIM, top_k=KNN_TOPK, p=KNN_P, dynamic=KNN_DYNAMIC)
            metodo_usado = "faiss_knn"
        else:
            estimate, _ = _estimate_with_softmax(neighbors_used, temperature=0.20)
            metodo_usado = "softmax"

    # Ajuste por cantidad de campos (si aplica)
    N = _extract_field_count(texto)
    if N > 0:
        cpf = _neighbor_based_cost_per_field(neighbors_used, labeled)
        alpha = 0.3
        base = alpha * float(estimate or 0.0) + (1.0 - alpha) * (cpf * N)
    ...
    # Top (presentación)
    top = []
    for (idx, sim, h) in sorted(neighbors_used, key=lambda t: t[1], reverse=True)[:3]:
        if idx is None or idx < 0 or idx >= len(labeled):
            continue
        row = labeled.loc[idx]
        top.append({
    ...
    return jsonify({
        "ok": True,
        "horas": float(round(estimate or 0.0, 4)),
        "metodo": metodo_usado,
        "top": top
    })
